...
### using jags output
mcmc_list_jags = readRDS("mcmc_list2.rds")
...
log_posterior <- function(samples.row, data) {
  ...
  beta_sum = dmnorm(beta, 0, diag(sqrt(1000),36), log=T)
  as.numeric(Likli_fun(samples.row, data) + eta_sum + rho_sum + tau_sum + sigma_sum + beta_sum)
}


cname <- colnames(mcmc_list_jags[[1]]$BUGSoutput$sims.array[,1,c(1:36, 38:61)])
...
saveRDS(model.bridge, "model_bridge_jags2.rds")
logml = rep(0, 24)
for(i in 1:24){
