...
gibbs_simpler <- function(Y1, Y2, X1, X2, n, q, cn, ns, udnei, sigmasq_beta, 
  ...
  print(accept_rate)
  return(list(par <- list(beta1 = beta1_t[,1], beta2 = beta1_t[,2], beta3 = beta1_t[,3], beta4 = beta2_t[,1], beta5 = beta2_t[,2],  
...
for(seed in 51:100){
  ...
  set.seed(seed)
  tod1=Sys.time()  
  result <- gibbs_simpler(Y1=Y1new, Y2=Y2new, X1=X1new, X2=X2new, n=n, q=q, cn=cn, ns=ns, udnei=udnei, sigmasq_beta=1000, Minc = Minc, 
  ...
  Y1=Y1new; Y2=Y2new; X1=X1new; X2=X2new
  ...
  for(i in 1:15000){
    ...
    W1vec <- result[[2]]$W1[i,]
    W2vec <- result[[2]]$W2[i,]
    beta1 <- as.vector(theta[1:3])
    beta2 <- as.vector(theta[4:5])
    sigmasq1 <- as.numeric(theta[6])
    sigmasq2 <- as.numeric(theta[7])
    
    KL_dagar[i] = kl.norm(mu1 = X%*%beta+W, S1 = sigma^2, mu2 = X%*%unlist(c(beta2, beta1)) + c(W2vec,W1vec), 
                          S2= as.matrix(bdiag(sigmasq2*diag(n), sigmasq1*diag(n))))
  }
