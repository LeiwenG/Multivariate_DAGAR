...
for(seed in 51:100){
  ...
  model.data <- list(k = n, I = diag(n), I1 = diag(5), Minc = Minc, D = diag(rowSums(Minc)), 
                     X = as.matrix(bdiag(X1new, X2new)), Y = c(Y1new, Y2new))
  ...
  for(i in 1:15000){
    ...
    KL_dagar[i] = kl.norm(mu1 = X%*%beta+W, S1 = sigma^2, mu2 = as.vector(X%*%unlist(c(beta2, beta1))) + 
                            c(as.numeric(W2_mcmc[i,]), as.numeric(W1_mcmc[i,])), 
