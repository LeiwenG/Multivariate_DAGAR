## Takes northeast as an example. You'll find code to change ordering.
...
for(seed in 1:100){
  ...
  ### changes to run the DAGAR model with a new ordering
  newperm=order(-albersproj$x-albersproj$y) ## x-y is for ordering starting at northwest, y-x starts at southeast, -x-y at northeast
  Minc=Minc[newperm,newperm]
  ...
  Y2 = Y[-(1:n)]
  ### changes to run the DAGAR model with a new ordering
  ## use the new ordering to permute the data 
  X1=X1[newperm,]
  X2=X2[newperm,]
  X = as.matrix(bdiag(X1, X2))
  ...
  for(i in 1:15000){
    ...
    for(j in 1:n){
      ...
      v1 <- sigmasq1
      PL_single[j,i] <- as.numeric(-1/2*log(2*pi) - 1/2*log(v1) - 1/(2*v1)*(Y1[j]-X1[j,]%*%t(as.vector(beta1)) - W1vec[j])^2)
      v2 <- sigmasq2
      PL_single[(j+n),i] <- as.numeric(-1/2*log(2*pi) - 1/2*log(v2) - 1/(2*v2)*(Y2[j]-X2[j,]%*%t(as.vector(beta2)) - W2vec[j])^2)
    }
  ...
  for(i in 1:15000){
    ...
    z1 <- rnorm(48, 0, 1)
    z2 <- rnorm(48, 0, 1)
    Y_rep1[,i] <- X1 %*% t(as.vector(beta1)) +  result[[2]]$W1[i,] + sqrt(as.numeric(sigmasq1)) * z1[newperm]
    Y_rep2[,i] <- X2 %*% t(as.vector(beta2)) +  result[[2]]$W2[i,] + sqrt(as.numeric(sigmasq2)) * z2[newperm]
  ...
  saveRDS(D_score, paste("D2_", modelgraph, ".rds", sep=""))
